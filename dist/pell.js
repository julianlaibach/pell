(function (global, factory) {
	...
	(global = global || self, factory(global.pell = {}));
	...
	const createColorPopup = (classes, settings) => {
	  ...
	  let colors = ['#f4f4f4', '#f6f6f6', '#999999'];
	...
	const init = settings => {
	  const actions = settings.actions ? settings.actions.map(action => {
	    if (typeof action === 'string') return defaultActions[action];else if (defaultActions[action.name]) return {
